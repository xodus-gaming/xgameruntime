...
static HRESULT WINAPI x_game_XGameGetXboxTitleId( IXGameImpl3 *iface, UINT32 *titleId )
{
    ...
    /* <TitleId>663E2626</TitleId> -- hexadecimal, no 0x prefix. */
    if (!(value = xodus_game_config_value( "TitleId" ))) return E_GAMEUSER_NO_TITLE_ID;

    id = strtoul( value, NULL, 16 );
    free( value );
    if (!id) return E_GAMEUSER_NO_TITLE_ID;
